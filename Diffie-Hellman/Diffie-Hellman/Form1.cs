...
namespace Diffie_Hellman
{
    public partial class Form1 : Form
    {
        ...
        public bool Kontrollo(BigInteger num, BigInteger p)
        {
            if (num >= p)
            {
                return false;
            ...
            {
                BigInteger[] vargu = new BigInteger[(int)p - 1];
                BigInteger[] vargu2 = new BigInteger[(int)p - 1];
                ...
                for (int i = 1; i <= (p - 1); i++)
                {
                    vargu[i - 1] = BigInteger.ModPow(num, i, p);
